    ...
    try {
      ...
      if (oauthError) {
        ...
        if (msg.toLowerCase().includes("not enabled") || msg.toLowerCase().includes("unsupported provider")) {
          setError(
            "Google sign-in is not yet enabled. In Supabase: Authentication → Providers → enable Google and add your OAuth credentials."
          );
        } else {
    ...
    <div className="w-full max-w-sm space-y-6">
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-muted-foreground text-sm leading-relaxed">
          Sign in to access your price tracker. Your saved products and alerts will be right here.
        </p>
      </div>
      ...
      <p className="text-center text-xs text-muted-foreground leading-relaxed">
        First time here? An account is created automatically when you sign in.
        <br />
